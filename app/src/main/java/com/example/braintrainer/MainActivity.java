...
public class MainActivity extends AppCompatActivity {
    ...
    private void generateQuestion() {
        int a = (int) (Math.random() * (max - min + 1) + min); //получим числа от 5 до 25
        int b = (int) (Math.random() * (max - min + 1) + min); //получим числа от 5 до 25
        int mark = (int) (Math.random() * 2); //получим 0 или 1 - для того чтобы узнать + или - будет в примере
        isPositive = mark == 1; //если mark == 1 то будет true, если нет то false
        if (isPositive) {
            rightAnswer = a + b;
    ...
    private int generateWrongAnswer() {
        int result;
        do {
            result = (int) (Math.random() * max * 2 + 1) - (max - min); //число от -25 до 35
            //(5-30=-25) (5+30=35)
